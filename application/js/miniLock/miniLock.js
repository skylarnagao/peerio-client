...
//	 and sent to the specified callback.
    miniLock.crypto.encryptFile = function (file,
                                            saveName,
                                            miniLockIDs,
                                            myMiniLockID,
                                            mySecretKey,
...
//			senderID: Sender's miniLock ID (Base58 string)
    miniLock.crypto.encryptNextChunk = function (file,
                                                 streamEncryptor,
                                                 hashObject,
                                                 encryptedChunks,
                                                 dataPosition,
...
// Result: Returns a header ready for use by a miniLock file.
    miniLock.crypto.createHeader = function (miniLockIDs,
                                             myMiniLockID,
                                             mySecretKey,
                                             fileKey,
                                             fileNonce,
...
// Result: Returns decrypted decryptInfo object containing decrypted fileInfo object.
    miniLock.crypto.decryptHeader = function (header, mySecretKey, myMiniLockID) {
        // removing for now, will be replaced by proper semver string + handler routing
        //if (
        ...
        if (
            !actualDecryptInfo
            || !hasProp(actualDecryptInfo, 'recipientID')
            || actualDecryptInfo.recipientID !== myMiniLockID
        ) {
            return false
        }
        if (
            !hasProp(actualDecryptInfo, 'fileInfo')
            || !hasProp(actualDecryptInfo, 'senderID')
...
//	and sent to the specified callback.
    miniLock.crypto.decryptFile = function (file,
                                            myMiniLockID,
                                            mySecretKey,
                                            callback) {
...
//			saveName: File name for saving the file (String),
//			senderID: Sender's miniLock ID (Base58 string)
    miniLock.crypto.decryptNextChunk = function (file,
                                                 streamDecryptor,
                                                 hashObject,
                                                 decryptedChunks,
                                                 dataPosition,
