    ...
    Peerio.UI.controller('todosSection', function () {
        ...
        var TodoList = React.createClass({
            ...
            render: function () {
                ...
                if (!todoList) return React.createElement(
                    'div',
                    { className: 'note-not-selected' },
                    React.createElement(
                        'h1',
                        null,
                        l('noTodoListSelected')
                    ),
                    React.createElement(
                        'p',
                        null,
                    ...
                    React.createElement(
                        'button',
                        { onClick: () => Peerio.TODOs.create() },
                        l('addMyFirstTodoList')
                    )
                );
                ...
                return React.createElement(
                    'div',
                    { className: 'note' },
                    React.createElement('input', { type: 'text', className: 'note-name', value: todoList.name, onChange: this.onNameChange }),
                    React.createElement(
                        'div',
                        { className: 'new-todo-input-prompt' },
                        'What has to be done?'
                    ),
                    React.createElement('input', { type: 'text', className: 'new-todo-input', value: this.state.newTodo,
                        onChange: this.onNewTodoChange, onKeyDown: this.onNewTodoKeyDown }),
                    React.createElement(
                        'div',
                        { className: 'todo-list-items' },
                        todoList.list.map(i => React.createElement(TodoListItem, { key: i.id, item: i, onChange: this.onChange, onRemove: this.onRemove }))
                    )
                );
            }
        });
            ...
            render: function () {
                return React.createElement(
                    ...
                    Peerio.user.todos.map(function (todoList) {
                        return React.createElement(TodoListListItem, { key: todoList.id, id: todoList.id, name: todoList.name,
