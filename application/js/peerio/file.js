...
(function () {
    ...
    Peerio.file.uploadGhost = function (file, ghostRecipientKey, onProgress, onFinish) {
        console.log('uploading ghost file '+file.name);
        var clientFileID = Base58.encode(nacl.randomBytes(32));
        var chunkNumber = 0;
        var totalChunks = Math.ceil(file.size / miniLock.crypto.chunkSize);
        var chunks;
        var id = '';
        var uploadFileComplete = false;
        var ghostFile;
        Peerio.crypto.encryptFile(file, {ghost: ghostRecipientKey},
            function (fileName) {
        ...
        var sendNextChunk = function (chunk, failed) {
            ...
            if (chunkNumber === 0) {
                chunkObject.header = ghostFile.header
            }
