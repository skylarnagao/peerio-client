            ...
            render: function () {
                ...
                if (!pass)
                    return <div className="note-not-selected">
                        <h1>{l('noPasswordsSelected')}</h1>
                        <p>{l('noPasswordsText')}</p>
                        <button onClick={()=>Peerio.Passwords.create()}>{l('addMyFirstPassword')}</button>
                    </div>;
                //<span class="loginShowPassphraseEnable fontAwesome">&#xf06e;</span>
                //<span class="loginShowPassphraseDisable fontAwesome">&#xf070;</span>
                return <div className="note">
                    <input type="text" className="note-name" value={pass.name} onChange={this.onNameChange}/>
                    <div className="pwd-title">Username:</div>
                    <input type="text" className="pwd-username" value={pass.username} onChange={this.onUsernameChange}/>
                    <div className="pwd-title">Password:</div>
                    <div className="pwd-secret-container">
                        <span className="fontAwesome pwd-toggle"
                              onClick={this.toggleHidden}>{this.hidden ? '\uf06e' : '\uf070'}</span>
                        <input type={this.state.hidden?'password':'text'} className="pwd-secret" value={pass.secret}
                               onChange={this.onSecretChange}/>
                    </div>
                    <div className="pwd-title">Notes:</div>
                    <textarea value={pass.note} className="pwd-note" onChange={this.onNoteChange}/>
                </div>;
            }
        });
        //--------------------------------------------------------------------------
                ...
                return <div className="note-list">
                    ...
                    </div>
                    {
                        Peerio.user.passwords.map(function (pass) {
                            return <PasswordListItem key={pass.id} id={pass.id} name={pass.name}
